  ...
  <div class="personal utBorder" :class="{'personal--white':UTstyle =='white','personal--black':UTstyle =='black'}">
    <div class="personal__box">
      <div class="personal__quanzi" v-show="footTab == 0">
        ...
        <div class="quanzi__show" v-show="quanziTab==0">
          <div class="show__item" v-for="(item, index) in firendlist" :key="index">
            <div class="item__userphoto">
               <img :src="item.friendAvatar" alt="">
            </div>
            <div class="item__username">{{item.friendPenName}}</div>
            <div class="item__usermsg">{{item.friendSiteName}}</div>
          </div>
        </div>
        <div class="quanzi__show" v-show="quanziTab==1">
          <div class="show__item">
           ...
           <div class="quanzi__show" v-show="quanziTab==2">
          <div class="show__item" v-for="(item, index) in followMeList" :key="index">
            <div class="item__userphoto">
              <img :src="item.userAvatar" alt="">
            </div>
            <div class="item__username">{{item.penName}}</div>
            <div class="item__usermsg">石头的话</div>
          </div>
        </div>
            <div class="quanzi__show" v-show="quanziTab==3">
          <div class="show__item" v-for="(item, index) in followsList" :key="index">
            <div class="item__userphoto">
              <img :src="item.userAvatar" alt="">
            </div>
            <div class="item__username">{{item.penName}}</div>
      ...
      <div class="personal__happen" v-show="footTab == 1">
        ...
        <div class="happen__show" v-show="happenTab==0">
          <div class="show__item" v-for="(item, index) in friendDiscover" :key="index">
            <div class="item__userphoto">
              <img :src="item.userAvatar" alt="">
            </div>
            <div class="item__username">{{item.userName}}</div>
            <div class="item__usermsg">{{item.msg}}</div>
          </div>
        </div>
        <div class="attention__show" v-show="happenTab==1">
          <div class="attention__item" v-for="(item, index) in followMe" :key="index">
            <div class="item__userphoto">
              <img :src="item.userAvatar" alt="">
            </div>
            <div class="item__username">{{item.userName}}
              <span class="item__time">{{item.createTime}}</span>
            </div>
            <div class="item__usermsg">{{item.msg}}</div>
          </div>
        </div>
          ...
          <div class="person__btn">
            ...
            <div class="person__buyplan" v-show="personTab == 0">
              <div class="buyplan__books" >
               ...
               <!-- <div v-if="bookbuy.rows.length==0" style="height: 100%; width: 100%;text-align: center;margin: 60px auto;">暂无数据</div> -->
            </div>
            ...
            <div class="person__lastvisited" v-show="personTab == 1">
              ...
              <div class="lastvisited__list" v-show="lastvisitedTab==0">
                <div class="list__item"  v-if="articlelist.length>0" v-for="(item, index) in articlelist" :key="index">
                  <div class="list__name">{{item.visitUserName}}</div>
                  <div class="list__author">{{item.targetName}}</div>
                </div>
                 <div v-if="articlelist.length==0" style="height: 100%; width: 100%;text-align: center;margin: 60px auto;">暂无浏览文章</div>
              </div>

              <div class="lastvisited__list" v-show="lastvisitedTab==1">
                <el-carousel class="books__list"  trigger="click" v-if="booklist.length>0" height="160px" indicator-position="none">
                    <el-carousel-item v-for="(item, index) in booklist" :key="index">
                  <div class="books__item fl">
                    <div class="books__img">
                      <img :src="booklist[index].visitAvatar" alt="">
                    </div>
                    <div class="books__name">{{booklist[index].visitUserName}}</div>
                  </div>
                  <div class="books__item fr">
                    <div class="books__img">
                      <img :src="booklist[index+1].visitAvatar" alt="">
                    </div>
                    <div class="books__name">{{booklist[index+1].visitUserName}}</div>
                  </div>
                    </el-carousel-item>
                </el-carousel>
                <div v-if="booklist.length==0" style="height: 100%; width: 100%;text-align: center;margin: 60px auto;">暂无浏览书籍</div>
              </div>
              <div class="lastvisited__list" v-show="lastvisitedTab==2">
                <div class="lastvisited__show" v-if="userlist.length>0"  v-for="(item, index) in userlist" :key="index">
                  <div class="show__item">
                    <div class="item__userphoto">
                 ...
                 <div v-if="userlist.length==0" style="height: 100%; width: 100%;text-align: center;margin: 60px auto;">暂无访问用户</div>
              </div>
            </div>
            ...
            <div class="person__btnend">
              ...
              <div class="person__utstatus" v-show="personTab == 5">
                <div class="utstatus__show" v-show="statusBtn==1">
                  <div class="show__item" v-for="(item, index) in openUsers" :key="index">
                    <div class="item__userphoto">
                      <img :src="item.userAvatar" alt="">
                    </div>
                    <div class="item__username">{{item.userName}}</div>
                    <div class="item__status"><a href='javascript:;' @click="open[index].flag=!open[index].flag">{{open[index].flag?'开放':'不开放'}}</a></div>
                  </div>
                </div>
        ...
        methods: {
            ...
            setpassword() {
                if (
                    ...
                    /^[a-zA-Z0-9]{6,20}$/.test(this.password)
                ) {
                    this.unitAjax(
                        "post",
                        "v1/me/passwordSetting", {
            ...
            //获取好友动态
            getfirendDiscover() {
                this.unitAjax('get', 'v1/me/friend/discover', {
                    userId: this.getValue('userId'),
                    page: '1',
                    pageSize: '20'
                }, res => {
                    if (res.code == 200) {
                        this.friendDiscover = res.data.rows;
                    }
                })
            },
            changeQuanziTab(idx) {
                ...
                switch (idx) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        this.unitAjax('get', 'v1/me/followMe', {
                            page: 1,
                            pageSize: 20,
                            userId: Number(this.getValue('userId'))
                        }, res => {
                            if (res.code == 200) {
                                this.followMeList = res.data.rows;
                            }
                        })
                        break;
                    case 3:
                        this.unitAjax('get', 'v1/me/follows', {
                            page: 1,
                            pageSize: 20,
                            userId: Number(this.getValue('userId'))
                        }, res => {
                            if (res.code == 200) {
                                this.followsList = res.data.rows;
                            }
                        })
                        break;
                    default:
            ...
            changeHappenTab(idx) {
                this.happenTab = idx;
                if (idx == 1) {
                    this.unitAjax('get', 'v1/me/follow/discover', {
                        userId: this.getValue('userId'),
                        page: '1',
                        pageSize: '20'
                    }, res => {
                        if (res.code == 200) {
                            this.followMe = res.data.rows;
                        }
                    })
                }
            },
        ...
        .books__list {
            ...
            .books__item {
                ...
                .books__img {
                    ...
                    &:hover {
                        .del {
                            background: url("../assets/images/userinfo/del-msg.png");
                        }
                    }
                    .del {
